						...
						<div class="tab-content">
							...
							<div class="tab-pane fade" id="tab_0">
								<!-- BEGIN EXAMPLE TABLE PORTLET-->
								 <jsp:include page="tabs/adsManagement-tab.jsp"></jsp:include> 
...
<script src="/EBike/backend/ads/js/adsManagement-table-ajax.js"></script>
<script>
var oAdsManagementTable;
var activeTab = "${activeTab}";
var adsManagementURL = "/EBike/backendads/queryMap";
jQuery(document).ready(function() {     
	   ...
	   $("#menu_2").text("广告管理");
    App.init();
    oAdsManagementTable =  AdsTableAjax.init();
    ...
    $("#adsManagement tbody tr").click( function( e ) {
        if ( $(this).hasClass('active') ) {
            $(this).removeClass('active');
        }
        else {
            oAdsManagementTable.$('tr.active').removeClass('active');
            $(this).addClass('active');
        } 
    });

    $('#adsManagement_addBtn').on( 'click', function () {
	       window.location.href = "/EBike/backendads/newAds";
	    } );
    
});

function editAds(queryParams) {
    var arr = queryParams.split(",");
    window.location.href = "/EBike/backendads/getAds?id="+arr[0];
}

function deleteAds(queryParams) {
   var arr = queryParams.split(",");
                  ...
                  main: {
                   ...
                   success: {
                     ...
                     callback: function() {
                    	    $.ajax({  
				                type : 'POST',  
				                url :"/EBike/backendads/delete" ,  
		                        data:{"id" : arr[0]} ,
				                dataType : 'json', 
				                async : false,  //同步
				                success : function(data) {  
				                	 window.location.href = "/EBike/backendads/adsManagement?activeTab=tab_0";
				                },  
				                error : function() {  
				                   //serverError3.show();
				                }  
