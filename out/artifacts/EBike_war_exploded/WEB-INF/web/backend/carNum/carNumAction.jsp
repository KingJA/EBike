					...
					<!-- BEGIN EXAMPLE TABLE PORTLET-->
						<div class="portlet-body">
						  ...
						  <div class="portlet-body form">
									<form <c:if test="${doType eq 'add'}">action="/EBike/backendcarNum/saveCarNumber"</c:if>  <c:if test="${doType eq 'edit'}">action="/EBike/backendcarNum/updateCarNumber"</c:if> method="post" id="form_carNum" class="form-horizontal">
										<div class="form-body">
											...
											<div class="form-group">
												<label class="control-label col-md-3">车牌号码
												<span class="required">
													 *
												</span>
												</label>
												<div class="col-md-4">
													<input type="text" name="carNum" data-required="1" class="form-control" value="<c:out value='${carNumber.carNum}'/>" />
													<input type="hidden" name="id" value="<c:out value='${carNumber.id}'/>"/>
												</div>
											</div>
										</div>
...
<script src="/EBike/backend/carNum/js/form-carNum-validation.js" ></script>
<!-- 上传文件 -->
...
jQuery(document).ready(function() {   
	...
	$("#carNumManageMenu").addClass("active");
	$("#menu_1").text("电动车管理");
	$("#menu_2").text("选号管理");
