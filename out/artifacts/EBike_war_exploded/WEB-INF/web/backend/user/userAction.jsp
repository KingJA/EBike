					...
					<!-- BEGIN EXAMPLE TABLE PORTLET-->
						<div class="portlet-body">
						  ...
						  <div class="portlet-body form">
									<form <c:if test="${doType eq 'add'}">action="/EBike/backenduser/saveUser"</c:if>  <c:if test="${doType eq 'edit'}">action="/EBike/backenduser/updateUser"</c:if> method="post" id="form_user" class="form-horizontal">
										<div class="form-body">
											...
											<div class="form-group">
												...
												<div class="col-md-4">
												<c:if test="${doType eq 'add'}"><input type="text" name="userName" data-required="1" class="form-control" value="<c:out value='${user.userName}'/>" /></c:if>
												<c:if test="${doType eq 'edit'}"><c:out value='${user.userName}'/></c:if>
											...
											<div class="form-group">
												...
												<div class="col-md-4">
													<input type="text" name="mobile" data-required="1" class="form-control" value="<c:out value='${user.mobile}'/>" />
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-md-3">身份证号
												<span class="required">
													 *
												</span>
												</label>
												<div class="col-md-4">
													<input type="text" name="certNo" data-required="1" class="form-control" value="<c:out value='${user.certNo}'/>" />
												</div>
											</div>
											<div class="form-group">
...
<script src="/EBike/backend/user/js/form-user-validation.js" ></script>
<!-- 上传文件 -->
...
jQuery(document).ready(function() {   
	...
	$("#userManageMenu").addClass("active");
	$("#menu_1").text("电动车管理");
	$("#menu_2").text("车主/用户管理");
	
	   $('#cancelBtn').on( 'click', function () {
	       window.location.href = "/EBike/backenduser/userManagement?activeTab=tab_0";
	    } );
